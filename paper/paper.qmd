...
```{r}
#| label: tbl-contract
#| tbl-cap: Summarizes the contract type which has over 30 contracts with the total dollar amount, and average dollar amount per contract. 
#| echo: false
#| eval: true
#| warning: false
#| message: false

# Summarize the data and filter buyers with counts >= 30
...
**Buyer** is a character-type variable representing the federal government department or agency that initiates and awards the contract. [@tbl-buyer] shows the buyer who has over 30 contracts with Microsoft, which identifies key partners and understands the distribution of contracts across sectors. The table highlights that Employment and Social Development Canada (ESDC) and National Defence account for over 50% of Microsoft's total awards.

```{r}
#| label: tbl-buyer
#| tbl-cap: Summarizes the buyers who has over 30 contract counts, with total dollar amount and average dollar amount per contract. 
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
The Full Bayesian Model explains 28.9% of the variance in contract amounts, as indicated by the $R^2$ value of 0.289. In comparison, the Reduced Model explains only 8.2% of the variance, highlighting the contribution of additional predictors (e.g., specific buyers and contract names) in improving the model's explanatory power. However, both models show low adjusted $R^2$ values, suggesting that the models do not capture a large portion of the unexplained variance in the data. This points to the need for further refinement, potentially by incorporating additional factors that influence procurement decisions.

The Full Bayesian Model has a Log-Likelihood of $-15,351.795$ and an ELPD of $-15,519.3$, while the Reduced Model's Log-Likelihood is $-15,398.022$ and ELPD is $-15,444.8$. The Full Model performs slightly better, as indicated by its higher Log-Likelihood and ELPD values. Both of these metrics suggest that the Full Model fits the data more accurately, as higher Log-Likelihood and ELPD values correspond to better data fit and prediction accuracy.

The Full Bayesian Model has a lower LOOIC ($31,038.7$) and WAIC ($30,992.3$) compared to the Reduced Model, suggesting better out-of-sample predictive performance. LOOIC and WAIC are used to estimate the out-of-sample prediction accuracy, with lower values indicating better model performance. These results further support the conclusion that the Full Model is superior in predicting contract amounts.

The Full Bayesian Model also outperforms the Reduced Model in terms of RMSE, with a value of $2,714,467.04$ compared to $2,971,135.35$ for the Reduced Model. The lower RMSE indicates that the Full Model is more accurate in predicting the total contract amount, making it a better tool for forecasting future contract values.

More results about the Bayesian model can be found in @sec-model-details. This includes further exploration of model convergence, posterior predictive checks, and alternative specifications that could be explored in future research.
...
```{r}
#| label: fig-amount
#| fig-cap: Total contract amounts awarded to Microsoft ($million) by year (since 2020). The data is grouped by award year, highlighting an increasing trends in Microsoft's federal contracts.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
#| fig-cap: Distribution of preparatory period. This shows the frequency of contracts grouped by the number of days spent in the preparatory phase, illustrating the variability in preparatory phase durations across the dataset.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
```{r}
#| label: fig-contract
#| fig-cap: Distribution of contract performance phase durations. The histogram shows the frequency of contracts grouped by the number of days in the performance phase, which demonstrates the days between start and end date.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
```{r}
#| label: fig-cor
#| fig-cap: Correlation matrix of numeric variables. This shows a weak negative correlations between $PreparatoryPhase$ and $ContractDays$; and weak positive correlations between predictors and $Amount$.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
```{r}
#| label: fig-box
#| fig-cap: The relationship of ContractDays and PhaseDays by ContractType. 
...
```{r}
#| label: fig-date
#| fig-cap: The distribution of federal contracts by Four-Month Periods (2020–2024). The heatmap shows the frequency of federal procurement contracts awarded to Microsoft in each four-month period, with darker colors representing higher contract frequencies.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
#| fig-cap: The relationship between Award Date and Start Date for the top 10 federal procurement contracts types. Each point represents a contract, with colors indicating different contract types. The plot helps to identify trends in the timing between contract award and commencement.
#| echo: false
#| eval: true
#| warning: false
#| message: false

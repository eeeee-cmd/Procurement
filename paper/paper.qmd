---
title: "Procurement Dynamics: Analyzing Federal Contract Awards to Microsoft and Bell"
...
```{r}
#| label: tbl-buyer
#| tbl-cap: The table summarizes top 10 buyer's name who has over 50 contracts with Microsoft in total with contract count, total dollar amount, and average dollar amount per contract. 
#| echo: false
#| eval: true
#| warning: false
#| message: false

# Summarize the data and filter buyers with counts >= 50
summary_table <- cleaned_data %>%
  group_by(Buyer) %>%
  summarise(
    Count = n(), 
...
**Supplier** is a character-type variable includes all the affiliated subsidiary companies of Microsoft. [@tbl-supplier] shows the top 5 buyer's name who has awarded over 50 contracts. [DELETE THIS VARIABLE? OR MAYBE MOVE IT TO APPDX.]
...
$$Amount_i = \beta_0 + \beta_1 ContractType + \beta_2 Buyer + \beta_3 ContractDays + \beta_4 PhaseDays + \epsilon_i$$
...
# load data and model
lm_model_m <- readRDS(here::here("models/lm_model_microsoft.rds"))
# lm_model_b <- readRDS(here::here("models/lm_model_bell.rds"))

# model_list <- list("Microsoft Model" = lm_model_m, "Bell Model" = lm_model_b)
modelsummary(model_list, 
             output = "kableExtra", 
             statistic = "std.error", 
             coef_map = c("ContractType" = "Contract Type", 
                          "Buyer" = "Buyer", 
...
# Access the OOB error rate
rf_model$oob.times  # Times each observation was used in the model
